...
exports.profile = (req, res,) => {
    ...
    sql.query('SELECT id, nom, prenom, ville, avatar_url, role, date_naissance, TIMESTAMPDIFF(YEAR, date_naissance, CURDATE()) AS age FROM user WHERE id=?', userId, function (error, results, fields) {
        if (error) {
            return res.status(500).json({ error });
